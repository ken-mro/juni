...
- **リザルトの遷移**: 終了 → `FINISH_TIME` 秒の告知（フィールド中央のドット文字。この間はリザルトが出ないので誤タップしない） → リザルト。クリア「次のレベルへ」と時間切れ「もう一度」は `startLevel()` で開始（タイトルを経由しない。READY → GO! の間は `game.started` が false で、隕石・タイマー・入力すべて止まっている）。「タイトルへ」と一時停止「やり直す」は `goToTitle()`。記録閲覧（view）の「閉じる」はオーバーレイを閉じるだけ（下にタイトルが残っている）

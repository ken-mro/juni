...
10. **ゲーム状態・入力・UI配線**: `game`（`timeLeft`/`level`/`destroyed`/`required`/`combo`）、統計 `stats`、記録 `juni.records`、`showResult(rec, mode)`（clear / fail / view の3モード）、`finishGame(cleared)`、`startLevel(level)`（READY → GO! の `countdown` を仕込み、`updateCountdown(dt)` が `CONFIG.READY_TIME` + `GO_TIME` 秒後に `beginPlay()` で `game.started = true` と BGM 開始）。終了時は `showFinish(phase, then)` が同じ `#countdown` に CLEAR! / TIME UP / THANK YOU のドット文字を出し、`CONFIG.FINISH_TIME` 秒後に `then()`（`showResult` と紙吹雪・記録更新音）を呼ぶ（終了直後の誤タップでリザルトのボタンを押さないための間。記録の保存自体は終了時点で済ませる）、開始レベル選択、記録一覧、共有画像 `buildShareImage` / `shareResult`
...
- **開始レベル**: 解放上限 = `maxClearedLevel() + 1`（上限なし）。選択肢は最低 `START_LEVEL_SHOWN` 個、解放が超えたぶんだけ増え、枠(`--lv-rows` 段)内でスクロール（スクロールバーは非表示。マスの幅はスクロールの有無で変えない — `goToTitle()` は非表示のうちに描画するので、表示後の再描画で幅が変わる作りにしない）。`startLevel(level)` は `settings.startLevel` も更新する
- **記録**: クリア時のみ更新。`juni.records[level]` はベストタイム更新（初クリア含む）のときだけ丸ごと置き換える。時間切れは記録に触れない。行別正答率の ↑↓ は同レベルの前ベストとの比較
- **リザルトの遷移**: 終了 → `FINISH_TIME` 秒の告知（フィールド中央のドット文字。この間はリザルトが出ないので誤タップしない） → リザルト。「次のレベルへ」（クリア・隠しステージ）と「もう一度」（閲覧以外の全モード。時間切れだけ主ボタンで、それ以外は「次のレベルへ」の下に `.secondary` の控えめな塗り）は `startLevel()` / `startSecret()` で開始（ボタンのアイコンはインライン SVG `.btn-ico`）（タイトルを経由しない。READY → GO! の間は `game.started` が false で、隕石・タイマー・入力すべて止まっている）。「タイトルへ」と一時停止「やり直す」は `goToTitle()`。記録閲覧（view）の「閉じる」はオーバーレイを閉じるだけ（下にタイトルが残っている）

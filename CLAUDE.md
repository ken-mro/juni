...
6. **レベル**: `LEVELS`（行の解放が難易度の主軸）と `CONFIG.LEVEL_LEN`（読みの文字数の範囲。L1 は2文字だけ、L5 まで段階的に長く、L6 以降は制限なし）の両方で絞った `LEVEL_POOLS`（起動時確定。`poolIndex(level)` で引く）。`speedScale(level)` / `spawnScale(level)` / `spawnIntervalFor(level)` / `requiredKills(level)` / `lenRange(level)` はレベルを引数に取る純関数。L6以降は速度と出現頻度が上がり続け、規定数も増える

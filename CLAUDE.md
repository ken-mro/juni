...
6. **レベル**: `LEVELS`（行の解放が難易度の主軸）と `CONFIG.LEVEL_LEN`（読みの文字数の範囲。L1〜L5 は1文字 = かな単体 `SINGLES`（`FLICK_MAP` のかなから生成。ん・を・ー・ゔ は出さず、小文字は L5 だけ）、L6 から `WORDS` の単語になり、2文字だけ → 最大が1文字ずつ増えて L11 以降は制限なし）の両方で絞った `LEVEL_POOLS`（起動時確定。`poolIndex(level)` で引く）。`speedScale(level)` / `spawnScale(level)` / `spawnIntervalFor(level)` / `requiredKills(level)` / `lenRange(level)` はレベルを引数に取る純関数。速度・出現間隔は L1〜L5 据え置き（`LEVEL_SPEED` / `LEVEL_SPAWN` のテーブル。L6 から上がる）で、テーブルを超えたレベルは末尾から `ENDLESS_*` で上がり続け、規定数も増える

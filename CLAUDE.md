...
6. **レベル**: `LEVELS`（行の解放が難易度の主軸。L1 あ・か・さ → L2 全清音 → L3 同じ字種で単語 → L4 濁音・半濁音 → L5 小文字）と `CONFIG.LEVEL_LEN`（読みの文字数の範囲。L1・L2 は1文字 = かな単体 `SINGLES`（`FLICK_MAP` のかなから生成。ん・を・ー・小文字・ゔ は出さない）、L3 から `WORDS` の単語になり、2文字 → L5 で2〜3文字 → 最大が1文字ずつ増えて L10 以降は制限なし）の両方で絞った `LEVEL_POOLS`（起動時確定。`poolIndex(level)` で引く）。`speedScale(level)` / `spawnScale(level)` / `spawnIntervalFor(level)` / `requiredKills(level)` / `lenRange(level)` はレベルを引数に取る純関数。速度・出現間隔は L1〜L5 据え置き（変える変数を「入力の技能」だけにするため。`LEVEL_SPEED` / `LEVEL_SPAWN` のテーブルで L6 から上がる）で、テーブルを超えたレベルは末尾から `ENDLESS_*` で上がり続け、規定数も増える
